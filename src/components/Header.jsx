...
import { FaWallet, FaCoins, FaHome, FaClipboardList, FaBriefcase, FaFolder, FaComments, FaShieldAlt, FaBell, FaUserCircle } from 'react-icons/fa'
...
const Header = () => {
  ...
  const navLinkClasses = (path) => {
    return `font-medium px-3 py-2.5 rounded-lg transition-all duration-300 flex items-center gap-2 ${isActive(path) 
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 flex justify-between items-center">
        ...
        <nav className="items-center space-x-1.5 md:flex hidden" aria-label="Main navigation">
          <Link to={'/'} className={navLinkClasses('/')} aria-label="Home page">
            <FaHome className={`${isActive('/') ? 'text-primary-500' : 'text-gray-400'}`} />
            <span>Home</span>
          </Link>
          <Link to={'/mybids'} className={navLinkClasses('/mybids')} aria-label="My Bids page">
            <FaClipboardList className={`${isActive('/mybids') ? 'text-primary-500' : 'text-gray-400'}`} />
            <span>My Bids</span>
          </Link>
          <Link to={'/myjobs'} className={navLinkClasses('/myjobs')} aria-label="My Jobs page">
            <FaBriefcase className={`${isActive('/myjobs') ? 'text-primary-500' : 'text-gray-400'}`} />
            <span>My Jobs</span>
          </Link>
          <Link to={'/myprojects'} className={navLinkClasses('/myprojects')} aria-label="My Projects page">
            <FaFolder className={`${isActive('/myprojects') ? 'text-primary-500' : 'text-gray-400'}`} />
            <span>My Projects</span>
          </Link>
          <Link to={'/messages'} className={navLinkClasses('/messages')} aria-label="Messages page">
            <FaComments className={`${isActive('/messages') ? 'text-primary-500' : 'text-gray-400'}`} />
            <span>Messages</span>
            {/* Notification badge example - can be made dynamic */}
            <span className="inline-flex items-center justify-center w-5 h-5 ml-1 text-xs font-medium text-white bg-accent-500 rounded-full animate-pulse-slow">2</span>
          </Link>
        ...
        <div className="md:flex hidden">
          {connectedAccount ? (
            <div className="flex items-center space-x-3">
              ...
              <button className="p-2 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600 hover:text-primary-600 transition-all duration-300" aria-label="User profile">
